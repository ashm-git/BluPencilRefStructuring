...
namespace RefBOT.Controllers
{
    class AutoStructReferencesHelper
    {
        ...
        public string ImproveLocalConversionWithSubRef(string sRefOriginalContent, string sRefTaggedContent, ref Dictionary<string, string> dicLogInfo, bool bIsStructureOnlyAutor = false, bool bUseLocalLocationData = false)
        {
            ...
            File.AppendAllText(@"D:\Projects\RefBOTDev\Log\Log.txt", "Step: s1");
            try
            {
        ...
        public string ImproveLocalConversion(string sRefOriginalContent, string sRefTaggedContent, ref Dictionary<string, string> dicLogInfo, bool bIsStructureOnlyAutor = false)
        {
            try
            {
                ...
                //added by Dakshinamoorthy on 2020-Dec-17
                sRefTaggedContent = Regex.Replace(sRefTaggedContent, @"(?:<(authorQueryOpen|authorQuerySelf|authorQueryClose)>((?:(?!</?\1).)+)</\1>)", "");
        ...
        private string RefOriginalContentPreCleanup(string sRefOriginalContent)
        {
            try
            {
                ...
                sRefOriginalContent = Regex.Replace(sRefOriginalContent, @"(?:<(authorQueryOpen|authorQuerySelf|authorQueryClose)>((?:(?!</?\1).)+)</\1>)", HandleBP_Comment);
        ...
        public string ConvertGenericRef(string sRefOriginalContent, string sRefTaggedContent, ref Dictionary<string, string> dicLogInfo, bool bProcessFromAPI = false)
        {
            try
            {
                ...
                //    //if (AutoStructRefAPI.IdentifyRefElements(sRefOriginalContent, ref sTaggedContent_API))
                //    //{
                //    //    NormalizeSpaces(ref sTaggedContent_API);
                //    //    sRefTaggedContent = ConvertGenericRef(sRefOriginalContent, sTaggedContent_API, ref dicLogInfo, true);
                //    //}
                //    sRefTaggedContent = DoAutoStructRefAPI(dtRefData, sRefOriginalContent);
                //}
                #endregion
        ...
        private string DoAutoStructRefAPI(DataTable dtRefData_API, string sRefOriginalContent)
        {
            string sTaggedContent_API = string.Empty;
            try
            {
                if (AutoStructRefAPI.IdentifyRefElements(sRefOriginalContent, ref sTaggedContent_API))
                {
                    ...
                    {
                        if (Enum.IsDefined(typeof(RefSkipElements), dtRefData_API.Rows[i]["ElementType"].ToString()) == false)
                        {
        ...
        private string RemoveGroupElements(string sRefTaggedContent)
        {
            try
            {
                //updated by Dakshinamoorthy for RefBOT API on 2019-June-12
        ...
        private string IntroSpaceAtValidPosition(string sRefContent)
        {
            try
            {
                ...
                sRefContent = Regex.Replace(sRefContent, @"( [0-9]+;)((?:[0-9]+))(\([^\(\)]+\)[\:]?)([a-z]?[0-9]+[a-z]?)", "$1<split/>$2<split/>$3<split/>$4", RegexOptions.IgnoreCase);
        ...
        private bool IdentifyEditionNumber(ref string sRefContent)
        {
            try
            {
                ...
                sRefContent = Regex.Replace(sRefContent, @"(?:(?<=(?:[ ]|\>[\.,;]*|\()(?<!(?:\: )))(?:[0-9]+[ ]?(?:<sup>)?(?:st|nd|rd|th)\.?(?:</sup>)?(?:[ ]+[Rr]ev(?:ised)?\.?)?[ ]+(?:(?:[Ee]d(?:itio)?n?)\b|(?:[Ee]dit)\b)\.?(?:[,;\:\) ]*|(?=[<]))))",
                string.Format("<{0}>{1}</{0}>", "ldlEditionNumber", "$&"));
        ...
        private bool IdentifyPublisherLocation(ref string sRefContent)
        {
            try
            {
                ...
                //updated by Dakshinamoorthy on 201-Jan-11
    ...
    "<fullDateDDMMYYYY>$&</fullDateDDMMYYYY>");
        ...
        {
            bool bReturnValue = false;
            string sOrgPattern = @"\b(?:Centre|Agency|Communities|Institute|National|Research|Council)\b";
